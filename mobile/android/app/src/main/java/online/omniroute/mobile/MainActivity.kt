...
private enum class AppTab(val label: String) { HOME("Home"), PROVIDERS("Providers"), ROUTING("Routing"), USAGE("Usage"), SETTINGS("Settings") }

class MainActivity : ComponentActivity() {
    private val runtimeViewModel: RuntimeViewModel by viewModels {
        val policy = RuntimePolicy()
        RuntimeViewModelFactory(
            TermuxRuntimeManager(TermuxIntentCommandRunner(applicationContext), policy),
            OmniRouteClient("http://127.0.0.1:${policy.port}") { null },
        )
    }
                ...
                bottomBar = {
                    NavigationBar(Modifier.navigationBarsPadding()) {
                        AppTab.entries.forEach { item ->
                            NavigationBarItem(item == tab, { tab = item }, icon = { Text(item.label.take(1)) }, label = { Text(item.label) })
                        }
                    }
                },
            ) { padding ->
...
@Composable
private fun HomeScreen(status: RuntimeStatus, snapshot: GatewaySnapshot?, viewModel: RuntimeViewModel, modifier: Modifier = Modifier) {
    ...
    Column(modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), Arrangement.spacedBy(16.dp)) {
        if (!running && status.state == RuntimeState.UNAVAILABLE) OnboardingCard(viewModel::installAndStart)
        else GatewayCard(status, running, busy, viewModel)
        HealthCard(status, snapshot, viewModel)
        RuntimeCard(status)
    }
}

@Composable private fun OnboardingCard(onStart: () -> Unit) = Card(Modifier.fillMaxWidth()) {
    Column(Modifier.padding(20.dp), Arrangement.spacedBy(12.dp)) {
        ...
        Text("Local gateway: 127.0.0.1:20128")
        Button(onStart, Modifier.fillMaxWidth()) { Text("Get Started") }
    }
}

@Composable private fun GatewayCard(status: RuntimeStatus, running: Boolean, busy: Boolean, viewModel: RuntimeViewModel) = Card(Modifier.fillMaxWidth()) {
...
@Composable private fun HealthCard(status: RuntimeStatus, snapshot: GatewaySnapshot?, viewModel: RuntimeViewModel) = Card(Modifier.fillMaxWidth()) {
...
@Composable private fun RuntimeCard(status: RuntimeStatus) = Card(Modifier.fillMaxWidth()) {
    Column(Modifier.padding(20.dp), Arrangement.spacedBy(8.dp)) {
...
@Composable private fun ProvidersScreen(modelCount: Int?, modifier: Modifier = Modifier) = PlaceholderScreen(modifier, "Providers", "Live model discovery is connected to /v1/models. Provider administration is next.", listOf("Discovered models: ${modelCount ?: "—"}", "Connection testing", "Secure key management"))
@Composable private fun RoutingScreen(status: RuntimeStatus, modifier: Modifier = Modifier) = PlaceholderScreen(modifier, "Routing", "Gateway connectivity is live. Routing controls will be wired to the supported server configuration API.", listOf("Gateway state: ${status.state}", "Automatic failover", "Health scoring", "Circuit breaker"))
...
@Composable private fun SettingsScreen(status: RuntimeStatus, modifier: Modifier = Modifier) = PlaceholderScreen(modifier, "Settings", "Runtime and security controls.", listOf("OmniRoute runtime: managed", "Version: 3.8.49", "Gateway: 127.0.0.1:${status.port}", "Credentials: Android Keystore"))

@Composable private fun PlaceholderScreen(modifier: Modifier, title: String, description: String, rows: List<String>) {
    Column(modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), Arrangement.spacedBy(16.dp)) {
